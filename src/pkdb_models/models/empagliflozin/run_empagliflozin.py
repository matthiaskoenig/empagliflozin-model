"""Tool to run the empagliflozin model factory and simulation experiments."""
...
from pkdb_models.models.empagliflozin import EMPAGLIFLOZIN_PATH
from pkdb_models.models.empagliflozin.simulations import run_simulation_experiments, EXPERIMENTS
from sbmlutils.console import console

FACTORY_SCRIPT_PATH = EMPAGLIFLOZIN_PATH / "models" / "factory.py"
...
def _setup_custom_results_paths(results_dir: str):
    ...
    empagliflozin.RESULTS_PATH = custom_path
    empagliflozin.RESULTS_PATH_SIMULATION = custom_path / "simulation"
...
def _get_current_results_path():
    ...
    from pkdb_models.models.empagliflozin import RESULTS_PATH
    return RESULTS_PATH
...
def _list_available_experiments():
    ...
    console.print('[dim]Example: run_empagliflozin --action simulate --experiments "misc,Heise2015"[/dim]')
    console.print(
        '[dim]Or use "all" to run all experiments: run_empagliflozin --action simulate --experiments all[/dim]\n')
...
def main() -> None:
    ...
    console.rule("[bold cyan]EMPAGLIFLOZIN PBPK/PD MODEL[/bold cyan]", style="cyan")
    ...
    if options.results_dir:
        ...
        if options.action and options.action.lower() == "factory":
            console.print(f"[cyan]Factory will use model base path: {empagliflozin.MODEL_BASE_PATH}[/cyan]")
        else:
...
if __name__ == "__main__":
    """
    This script is intended to be run from the command line as 'run_empagliflozin'.
    ...
    1. Help:
       ...
       $ run_empagliflozin --help

    2. Run the Model Factory:
       Generates all SBML model files.
       $ run_empagliflozin --action factory

    3. Run Simulations:
       List available experiments:
       $ run_empagliflozin --action list_experiments

       Run all experiments:
       $ run_empagliflozin --action simulate --experiments all

    5. Run Everything:
       Runs factory and all simulations.
       $ run_empagliflozin --action all

       With custom results directory for figures:
       $ run_empagliflozin --action all --results-dir '/path/to/my/results'
    """
    main()